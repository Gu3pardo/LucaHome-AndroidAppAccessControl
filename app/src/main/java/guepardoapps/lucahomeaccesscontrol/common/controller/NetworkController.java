...
public class NetworkController {
    ...
    public boolean IsHomeNetwork(@NonNull String homeSSID) {
        if (_logger != null) {
            _logger.Debug("homeSsid: " + homeSSID);
        }
        ...
        if (activeNetwork != null) {
            if (activeNetwork.getType() == ConnectivityManager.TYPE_WIFI) {
                ...
                try {
                    ...
                    String errorString = (ex.getMessage() == null) ? "HomeSSID failed" : ex.getMessage();
                    _logger.Error(errorString);

                    return false;
                }
            } else {
                _logger.Warn("Active network is not wifi: " + String.valueOf(activeNetwork.getType()));
            }
        } else {
            _logger.Warn("activeNetwork is null!");
        }
        return false;
    }
